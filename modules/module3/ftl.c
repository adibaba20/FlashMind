...
void ftl_init(ftl_t *ftl) {
    ...
    for (int b = 0; b < NUM_BLOCKS; b++) {
        ...
        ftl->blocks[b].last_erase_tick = 0;   /* MODULE 3: block is "freshly erased" at time 0 */
        ftl->blocks[b].sim_temp_c   = 40;     /* MODULE 3: baseline ambient temp, °C           */
    ...
    ftl->sim_tick = 0;  /* MODULE 3: simulated clock starts at 0 */
}
...
int ftl_write(ftl_t *ftl, int lba, const char *data) {
    ...
    ftl->sim_tick++;  /* MODULE 3: every write advances simulated time */
...
int ftl_read(ftl_t *ftl, int lba, char *out_data) {
    ...
    ftl->total_reads++;
    ftl->sim_tick++;  /* MODULE 3: every read also advances simulated time */
    return 0;
}
...
int ftl_erase_block(ftl_t *ftl, int block_id) {
    ...
    ftl->total_erases++;
    ftl->sim_tick++;                      /* MODULE 3: erase also advances simulated time      */
    blk->last_erase_tick = ftl->sim_tick; /* MODULE 3: retention clock resets — block is fresh  */
    blk->sim_temp_c = 38 + (rand() % 6);  /* MODULE 3: re-jitter temp slightly each erase, 38-43°C */
    return 0;
}
