...
typedef struct {
    ...
    /* MODULE 3 ADDITIONS: fields the degradation model reads from.
     * last_erase_tick: simulated "time" (operation count) this block was
     *   last erased — used to compute retention time (how long data has
     *   sat unrefreshed, which is a real driver of read errors in NAND).
     * sim_temp_c: a simulated ambient temperature for this block, jittered
     *   slightly per snapshot — higher temperature accelerates charge
     *   leakage in real flash, so it's a real factor in error models too. */
    uint64_t last_erase_tick;
    int      sim_temp_c;
} block_t;
...
typedef struct {
    ...
    /* MODULE 3 ADDITION: a simple simulated clock. Every read/write/erase
     * ticks this forward by 1. It's not "wall clock time" — it's operation
     * count — but it plays the same role: it lets us measure how long ago
     * something happened (retention time = sim_tick - last_erase_tick). */
    uint64_t sim_tick;
} ftl_t;
...
/* ---------------------------------------------------------------------
 * MODULE 3 ADDITIONS: degradation model + telemetry logging
 * ---------------------------------------------------------------------
 * These functions don't change how the FTL behaves — they OBSERVE it.
 * Think of this as adding a "sensor" that watches each block's wear and
 * estimates how error-prone it's becoming, then writes that out as a
 * CSV row. That CSV is literally the training data for Module 4/5's ML
 * model — this module is the bridge between "systems simulator" and
 * "machine learning project".
 * ------------------------------------------------------------------- */
#define BITS_PER_PAGE          (PAGE_DATA_SIZE * 8)  /* 256 bits per toy page   */
#define ECC_CORRECTABLE_BITS   1    /* toy on-die ECC can fix up to 1 bit error per page;
                                        more than that = "uncorrectable" for this block */
#define BASE_RBER               0.00003  /* raw bit error rate of a totally fresh block */

/* degradation.c
 * Computes the estimated Raw Bit Error Rate for a block given its wear
 * (erase_count), how long since it was last erased (retention_ticks),
 * and its simulated temperature. Grows with all three — this mirrors
 * the general, well-known trend in real NAND flash reliability studies
 * (more P/E cycles, longer retention, and higher temperature all make
 * bit errors more likely), simplified into a toy formula for this sim. */
double deg_compute_rber(uint32_t erase_count, uint64_t retention_ticks, int temp_c);

/* degradation.c
 * Given an RBER, simulates how many of a page's BITS_PER_PAGE bits
 * actually flip, by testing each bit against the probability. This is
 * what turns a smooth "rate" into a realistic, noisy error COUNT — two
 * blocks with the same RBER won't always report the same error count,
 * just like real flash. */
int deg_simulate_bit_errors(double rber);

/* degradation.c
 * Writes the CSV header row. Call this once before logging any snapshots. */
void deg_log_header(FILE *f);

/* degradation.c
 * Takes one "sensor reading" of block_id's current health and appends
 * it as a CSV row to f. This is the function you call repeatedly over
 * the simulation's lifetime to build up a telemetry dataset. */
